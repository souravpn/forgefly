...
function buildDiffLines(
  ...
  for (const section of sections) {
      ...
      for (const svc of newSvcs) {
        if (!oldNames.has(svc.name)) {
          lines.push({
            type: "+",
            label: "service",
            detail: `${svc.name}${svc.price ? ` · ${svc.price}` : ""}`,
          });
        } else {
    ...
    if (section === "identity") {
      ...
      if (ni.businessName && oi.businessName !== ni.businessName) {
        lines.push({
          type: "~",
          label: "business name",
          detail: `${oi.businessName ?? "—"} → ${ni.businessName}`,
        });
      }
    ...
    if (section === "brand") {
      ...
      if (nb.primaryColor && ob.primaryColor !== nb.primaryColor) {
        lines.push({
          type: "~",
          label: "brand color",
          detail: `${ob.primaryColor ?? "—"} → ${nb.primaryColor}`,
        });
      }
    ...
    if (section === "contacts") {
      ...
      for (const c of newContacts) {
        if (c.name && !oldNames.has(c.name.toLowerCase())) {
          const detail = [c.name, c.email, c.phone, c.company]
            .filter(Boolean)
            .join(" · ");
    ...
    if (section === "proposal") {
      ...
      const op = asObj<Pr>(o);
      const np = asObj<Pr>(n);
      if (np.intro && op.intro !== np.intro)
        lines.push({ type: "~", label: "proposal intro", detail: "updated" });
      if (np.approach && op.approach !== np.approach)
        lines.push({
          type: "~",
...
export function CommandBar({
  ...
  async function handleSubmit() {
    ...
    if (!business) {
      toast.error(
        "No active business found — generate your Business OS first.",
      );
      return;
    }
    setIsLoading(true);
    ...
    try {
      ...
      if (pending.sections.includes("services")) {
        ...
        const oldNames = new Set(
          oldSvcs.map((s) => s.name?.toLowerCase()).filter(Boolean),
        );
        const toInsert = newSvcs.filter(
          (s) => s.name && !oldNames.has(s.name.toLowerCase()),
        );
      ...
      if (pending.sections.includes("contacts")) {
        ...
        const oldContacts = asArr<Contact>(
          (extractedData as RawData)?.contacts,
        );
        const newContacts = asArr<Contact>(
          (pending.mergedData as RawData)?.contacts,
        );
        const oldNames = new Set(
          oldContacts.map((c) => c.name?.toLowerCase()).filter(Boolean),
        );
        const toInsert = newContacts.filter(
          (c) => c.name && !oldNames.has(c.name.toLowerCase()),
        );
        if (toInsert.length > 0) {
          ...
          const { error: clientErr } = await supabase
            ...
            .insert(rows);
          if (clientErr)
            console.warn("Client sync warning (non-fatal):", clientErr);
        }
      }
  ...
  if (pending) {
    return (
      <div className="rounded-2xl border-2 border-primary/40 bg-background/60 overflow-hidden">
        <div className="px-4 pt-4 pb-2">
          <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide mb-1">
            Review changes
          </p>
          <p className="text-xs text-muted-foreground">
            These will be applied across all tabs of your OS.
          </p>
        </div>
        ...
        <ScrollArea className="max-h-64 px-4">
          <div className="space-y-0.5 py-2">
            {pending.lines.map((line, i) => (
              <div
                key={i}
                className="flex items-start gap-3 text-sm py-1.5 px-2 rounded-lg hover:bg-muted/40"
              >
                <span
                  className={cn(
                ...
                <span className="text-muted-foreground shrink-0 w-28 text-xs leading-relaxed">
                  {line.label}
                </span>
                {line.detail && (
        ...
        <div className="px-4 pb-4 pt-3 border-t flex gap-2">
          ...
          <Button
            onClick={applyDiff}
            disabled={applying}
            className="flex-1 gap-2 rounded-full bg-emerald-500 hover:bg-emerald-600 text-white"
          >
  ...
  // Input view
  return (
    <div className="rounded-2xl border-2 focus-within:border-primary bg-background/60 transition-colors px-4 pt-3 pb-3 relative overflow-hidden">
      {collapsed && (
        <>
          <img
            src="/upgrade-os-background.png"
            alt=""
            className="absolute inset-0 w-full h-full object-cover opacity-70 pointer-events-none"
          />
          {/* <p className="relative left-0 text-xs font-bold text-foreground">
            Make changes to your Business OS, with just a sentence(or two).
          </p> */}
        </>
      )}
      {!collapsed && (
        <Textarea
          ...
          placeholder="Describe the upgrades to your business here…"
          className="resize-none text-sm bg-transparent border-none shadow-none focus-visible:ring-0 px-0 min-h-[64px]"
          disabled={isLoading}
          onKeyDown={(e) => {
        ...
        <div className="flex items-center gap-2">
          ...
          {onToggleCollapse && (
            <Button
              ...
              aria-label={
                collapsed ? "Expand command bar" : "Collapse command bar"
              }
            >
              {collapsed ? (
          ...
          <Button
            ...
            className={cn(
              "gap-2 px-6 rounded-md font-semibold text-white",
          ...
          >
            {isLoading ? "Thinking…" : "Upgrade my Business"}
          </Button>
        </div>
      </div>
    </div>
