...
const CLIENTELE_NAV = [
  ...
  { id: 'reviews',  label: 'Reviews',  icon: 'star',           route: '/dashboard/reviews' },
]
...
function NavLink({ item, onClick }: { item: NavEntry; onClick?: () => void }) {
  ...
  return (
    <button
      type="button"
      onClick={() => { navigateTo(item.route); onClick?.() }}
      className={cn(
...
export function AppSidebar({ onNavigate, nudges: { nudges, unreadCount, markRead, markAllRead } }: AppSidebarProps) {
      ...
      <aside className="hidden md:flex flex-col w-64 bg-sidebar rounded-2xl border border-sidebar-border/40 shrink-0 overflow-hidden">
        ...
        <div className="px-4 py-4 border-b border-sidebar-border shrink-0">
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[11px] text-sidebar-foreground/50 mt-0.5 truncate">{tagline}</p>
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto px-2 py-2 space-y-0">
          ...
          <div className="pt-3 mt-3 border-t border-sidebar-border/60">
            <NavLink
              item={{ id: 'settings', label: 'Settings', icon: 'settings', route: '/dashboard/settings' }}
              onClick={onNavigate}
            />
            {slug && (
              <button
