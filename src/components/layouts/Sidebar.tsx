...
import { Home, Users, Briefcase, DollarSign, Calendar, FileText, Zap, Settings, LogOut, Menu, Package, Receipt, ChevronsLeft, ChevronsRight, Sun, Moon, Building2, HelpCircle, Bug, ChevronDown, Crown } from 'lucide-react';
  ...
  return (
    <>
      ...
      <div className={`p-6 border-b border-sidebar-border ${isCollapsed ? 'px-3' : ''}`}>
        <Link to="/dashboard" onClick={onNavigate} className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-3'}`}>
          <img 
          ...
          {!isCollapsed && (
            <div>
              ...
              <p className="text-xs text-sidebar-foreground/60">Forge Your Freedom</p>
            </div>
          )}
        </Link>
      </div>
          ...
          {!isCollapsed ? (
            <>
              ...
              >
                ...
                <ChevronDown className={`w-4 h-4 text-sidebar-foreground/60 transition-transform ${isUserMenuOpen ? 'rotate-180' : ''}`} />
              </button>
                  ...
                  <div className="p-2 space-y-1">
                    ...
                    {!isAgency ? (
                      ...
                      >
                        ...
                        <span className="text-sm text-sidebar-foreground">Switch to Agency Mode</span>
                      </button>
                    ) : (
                      <div className="flex items-center gap-3 px-3 py-2">
                        <Crown className="w-4 h-4 text-emerald-400" />
                        <span className="text-sm text-emerald-400 font-medium">Agency Mode Active</span>
                      </div>
                    )}
                    ...
                    <button className="w-full flex items-center gap-3 px-3 py-2 rounded-md hover:bg-sidebar-accent transition-colors text-left">
                      ...
                      <span className="text-sm text-sidebar-foreground">Help & Feedback</span>
                    </button>

                    {/* Report a Bug */}
                    <button className="w-full flex items-center gap-3 px-3 py-2 rounded-md hover:bg-sidebar-accent transition-colors text-left">
                      <Bug className="w-4 h-4 text-sidebar-foreground" />
                      <span className="text-sm text-sidebar-foreground">Report a Bug</span>
                    </button>
...
export function Sidebar() {
  ...
  return (
    <>
      ...
      <aside className={`hidden md:flex flex-col ${isCollapsed ? 'w-16' : 'w-64'} h-screen shrink-0 border-r border-sidebar-border transition-all duration-300`}>
        <SidebarContent 
