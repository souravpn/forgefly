...
function Step1({ onSubmit }: { onSubmit: (input: string) => void }) {
...
function Step2({ progress }: { progress: number }) {
  return (
    <div className="max-w-sm mx-auto space-y-6 py-8">
      ...
      <div className="w-full bg-muted rounded-full h-1.5 overflow-hidden">
        <div
          ...
          style={{ width: `${(progress / LOADING_STEPS.length) * 100}%` }}
        />
      </div>
    </div>
  );
      ...
      <div className="space-y-4">
        {activeTab === 'email' && (
          <div className="space-y-3">
            <div>
              ...
              <p className="text-sm bg-muted/40 rounded-lg p-3">{copyKit.cold_email.subject}</p>
            </div>
            <div>
              <div className="flex items-center justify-between mb-1">
              ...
              <p className="text-sm bg-muted/40 rounded-lg p-3 whitespace-pre-wrap leading-relaxed">{copyKit.cold_email.body}</p>
            </div>
          </div>
        )}
        ...
        {activeTab === 'dm' && (
          <div className="space-y-4">
            <div>
              ...
              <p className="text-sm bg-muted/40 rounded-lg p-3 whitespace-pre-wrap leading-relaxed">{copyKit.linkedin_dm}</p>
            </div>
        ...
        {activeTab === 'connect' && (
          <div>
            ...
            <p className="text-sm bg-muted/40 rounded-lg p-3 whitespace-pre-wrap leading-relaxed">{copyKit.connection_note}</p>
          </div>
        )}
        ...
        {activeTab === 'followup' && (
          <div className="space-y-3">
            <div>
              ...
              <p className="text-sm bg-muted/40 rounded-lg p-3">{copyKit.follow_up.subject}</p>
            </div>
            <div>
              <div className="flex items-center justify-between mb-1">
                <span className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Body</span>
              ...
              <p className="text-sm bg-muted/40 rounded-lg p-3 whitespace-pre-wrap leading-relaxed">{copyKit.follow_up.body}</p>
            </div>
          </div>
        )}
      </div>
...
function Step5({
  ...
  const [addedToSugar, setAddedToSugar] = useState(!!leadId);
...
export default function OutreachKitPage() {
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Research a target company and get a personalised multi-step sequence.
          </p>
        </div>
        {step > 1 && (
          <Button
      ...
      {step === 1 && <Step1 onSubmit={handleResearch} />}
      {step === 2 && <Step2 progress={loadingProgress} />}
      {step === 3 && intel && (
        <Step3
          intel={intel}
